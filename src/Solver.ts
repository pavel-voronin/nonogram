...
import SolverWorker from "./worker?worker&inline";
...
export default class Solver extends Nonogram {
  ...
  constructor(
    ...
    { theme = {}, delay = 50, onSuccess = () => {}, onError = () => {} } = {}
  ) {
    super();
  ...
  initListeners() {
    this.listeners = [["click", this.click as EventListener]];
  }
  click = (e: MouseEvent) => {
  ...
  printController() {
    ...
    function getCycle() {
      ...
      c.arc(
        0,
        0,
        controllerSize / 2 - borderWidth / 2,
        Math.PI / 2,
        Math.PI / 3.9
      );
