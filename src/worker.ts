...
class Solver {
  ...
  scan = () => {
    ...
    const finished = this.currentLine.every(
      (cell) => cell !== WorkerStatus.UNSET
    );
    if (!finished) {
  ...
  updateScanner() {
    ...
    do {
      ...
      if (this.hints[this.scanner.direction][this.scanner.i] === undefined) {
        this.scanner.direction =
          this.scanner.direction === "row" ? "column" : "row";
        this.scanner.i = 0;
      }
      line = this.hints[this.scanner.direction][this.scanner.i];

      if (
        this.hints.row.every((row) => !!row.unchanged) &&
  ...
  merge() {
    ...
    possibleBlanks.forEach((blanks, p) => {
      ...
      for (let i = 0; i < this.currentHints.length; i += 1) {
        ...
        line.push(
          ...new Array(this.currentHints[i]).fill(WorkerStatus.TEMP_FILLED)
        );
      }
      line.push(
        ...new Array(this.currentLine.length - line.length).fill(
          WorkerStatus.TEMP_EMPTY
        )
      );

      const improper = line.some(
        (cell, i) =>
          (cell === WorkerStatus.TEMP_EMPTY &&
            this.currentLine[i] === WorkerStatus.FILLED) ||
          (cell === WorkerStatus.TEMP_FILLED &&
            this.currentLine[i] === WorkerStatus.EMPTY)
      );
      if (improper) {
